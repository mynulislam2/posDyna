...
const Header = () => {
    ...
    return (
        <div className='flex items-center justify-between h-20 w-ful shadow-md px-10 sticky top-0 '>
            <input type="text" placeholder="Search here..." class="input input-bordered w-full max-w-xs" />
            <div className='flex items-center gap-8'>
                <button  onClick={()=>setislShowLanguage(!islShowLanguage)} className="relative flex items-center gap-3 h-10 text-lg border border-gray-light  py-2 px-2 rounded-xl text-gray-300">
                ...
                <Notifications display={islShowMessage} title='All Messages'/>
                <div className='flex content-between gap-1 relative'>
                    <svg className="" onClick={() => setisShowNotifi(!isShowNotifi)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <Notifications display={isShowNotifi} title='Notifications'/>
